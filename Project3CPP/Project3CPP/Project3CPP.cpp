...
void Transcript() {
	MYSQL_RES * res_set;
	string input = "";
	unordered_map<string, string> all_courses = {};
	char query[256];
	...
	printf("%-20s%-20s%-20s%-20s\n", "Course Number", "Year", "Grade", "Semester");
	int num_rows = (int)mysql_num_rows(res_set);
	int num_cols = (int)mysql_num_fields(res_set);
	MYSQL_ROW row;
	for (int i = 0; i < num_rows; i++) {
		row = mysql_fetch_row(res_set);
		if (row != NULL) {
			all_courses.insert({ string(row[0]),string(row[0])});
			printf("%-2d: ", i + 1);
			for (int j = 0; j < num_cols; j++) {
				printf("%-20s", row[j]);
			}
	...
	while (1) {
		while (all_courses.find(input) == all_courses.end()) {
			cout << "Input the course number (on the left) you want more information on" << endl;
			cout << "Input 'q' to quit" << endl;
			getline(cin,input);
			if (all_courses.find(input) == all_courses.end()) {
				cout << "Invalid course number enterred" << endl;
			}
	...
	while (1) {
		...
		for (int i = 0; i < num_rows; i++) {
			...
			if (row != NULL) {
				...
				printf("%-2d: ", i+1);
				for (int j = 0; j < num_cols; j++) {
					string tmp = string(row[j]);
					printf("%-20s", tmp.substr(0, 18).c_str());
				}
				cout << endl;
			}
		}
